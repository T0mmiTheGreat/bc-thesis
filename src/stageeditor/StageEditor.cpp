	...
	std::string& fn, bool& isAutoGenerated)
{
	...
	isAutoGenerated = true; // FIXME

	fn.clear();
	for (char c : stageTitle) {
		if (isalnum(c)) {
	...
	if (fn.empty()) {
		...
		fn = "stage";
	} else if (fn.size() > MAX_GENERATED_FN_LEN) {
		fn.resize(MAX_GENERATED_FN_LEN);
	}

	// FIXME: Check collisions with already existing file names
}
